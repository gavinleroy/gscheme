 ...
 *   | Lambda : ('b -> 'a) expr
 *   | Id : id -> id expr
 *   | App : ('b -> 'a) expr * 'b expr -> 'a expr
 *   | Quote : 'a datum -> 'a datum expr
 *   | LetSyntax : id expr * 'a expr -> 'b expr
 *   | QuoteSyntax : 'a datum -> 'a datum expr *)
...
(* and unwrap : type a. a typ -> dyn -> a option
 *   = fun a (Dyn(b,x)) -> match eq_type a b with
 *     | None -> None
 *     | Some Eq -> Some x *)

(********************************************
 ** Evaluation (for a different module) *)
...
let%test_module _ = (module struct
  ...
  (* Datum / syntax tests *)
  (* assert (datum_to_syntax (Dyn(IdT, Id "a"))
   *         = (Dyn(StxT, Stx ("a", Scopes.empty)))); *)
  let%test _ = (datum_to_syntax (Dyn(IntT, Int 1))
                = (Dyn(IntT, Int 1)))
      (* assert (datum_to_syntax (Dyn(ListT, List[ Dyn(IdT, Id "a")
       *                                         ; Dyn(IdT, Id "b")
       *                                         ; Dyn(IdT, Id "c")]))
      ...
      (* assert ((add_scope (Dyn(StxT, Stx ("x", Scopes.empty))) sc1)
       *         = (Dyn(StxT, Stx ("x", Scopes.singleton sc1)))); *)

      (* assert (add_scope (datum_to_syntax
       *                      (`List [`Id "x"; `List [`Id "y"]])) sc1
       *         = `List [`Stx ("x", Scopes.singleton sc1); `List [`Syntax ("y", Scopes.singleton sc1)]]);
       *
       * assert (add_scope (add_scope (`Syntax ("x", Scopes.empty)) sc1) sc2
       *         = `Syntax ("x", Scopes.of_list [sc1; sc2]));
       *
       * assert (add_scope (add_scope (`Syntax ("x", Scopes.empty)) sc1) sc1
       *         = `Syntax ("x", Scopes.singleton sc1));
       *
       * assert (flip_scope (`Syntax ("x", Scopes.singleton sc1)) sc2
       *         = `Syntax ("x", Scopes.of_list [sc1; sc2]));
       *
       * assert (flip_scope (`Syntax ("x", Scopes.of_list [sc1; sc2])) sc2
       *         = `Syntax ("x", Scopes.singleton sc1)); *)
